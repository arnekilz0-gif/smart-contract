...
// Provides standard ownership with owner(), onlyOwner, transferOwnership, renounceOwnership.
import { Ownable } from "@openzeppelin/contracts/access/Ownable.sol"; 
// nonReentrant modifier. Prevents reentering on functions that use nonReentrant and that send ETH or call external contracts.
import { ReentrancyGuard } from "@openzeppelin/contracts/security/ReentrancyGuard.sol"; 
// Emergency mechanism to stop contract. Mechanism with whenNotPaused/ whenPaused.
...
contract ParkingPPX is Ownable, ReentrancyGuard, Pausable { // All three modules are applied.
    ...
    // Finite state machine for the parking spot (FREE: spot is unused; CHECKED_IN: driver deposited, contract waiting for sensor confirmation; OCCUPIED: driver is parked, contract is tracking time).
    ...
    // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/contracts.html#function-modifiers] and is used to change the behaviour of functions in a declarative way. 
    // In Addition code is taken from here [https://docs.soliditylang.org/en/latest/contracts.html#custom-errors] to add a custom error that shows when the condition is false. 
    modifier onlyOracle() {
    ...
    // Subsequent code is taken from her [https://docs.openzeppelin.com/contracts/5.x/api/access#Ownable] and sets the intial owner.
        ...
        // Subsequent code is taken from here [https://docs.soliditylang.org/en/latest/contracts.html#custom-errors].
        require(_oracle != address(0), "zero oracle"); // It now requires that the oracle address is not zero. Zero Oracle would permanently disable oracle restricted flows.
    ...
    // The following functions follow this code [https://docs.soliditylang.org/en/latest/contracts.html#functions]. 
    // Code from [https://docs.soliditylang.org/en/latest/contracts.html#function-visibility] and [https://docs.openzeppelin.com/contracts/5.x/api/access#Ownable] is used to change visibility and add access modifier.
    function pause() external onlyOwner { // Owner can stop actions that are marked with whenNotPaused.
        // Code is taken from [https://docs.openzeppelin.com/contracts/5.x/api/utils#Pausable].
        _pause(); 
    }
    ...
    function setOracle(address _oracle) external onlyOwner { // Changes the orcale key if key is compromised or change needed.
        // Code is taken from here [https://docs.soliditylang.org/en/latest/contracts.html#custom-errors].
        require(_oracle != address(0), "zero oracle"); // It now requires that the oracle address is not zero. Zero Oracle would permanently disable oracle restricted flows.
        oracle = _oracle;
    }
    ...
    function setCheckInTimeout(uint256 _timeoutSec) external onlyOwner { // Changes the timeout and avoids absurd values that will break flow of the contract.
        // Code is taken from here [https://docs.soliditylang.org/en/latest/contracts.html#custom-errors].
        require(_timeoutSec >= 60, "timeout too small"); // This requires that the minimum timeout can not be smaller than 60 seconds.
    ...
    // Subsequent code is taken from here [https://docs.soliditylang.org/en/v0.8.34/types.html#address]. The function can send Ether to a plain address.
        ...
        whenNotPaused // Code from [https://docs.openzeppelin.com/contracts/5.x/api/utils#Pausable] to make the function only callable when not paused.
    {
        ...
        // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/control-structures.html#error-handling-assert-require-revert-and-exceptions] to transfer the amount to the specified address.
    ...
    // Subsequent Code is taken from [https://docs.soliditylang.org/en/latest/contracts.html#function-visibility] to set external visibility.
    // Subsequent Code is taken from  [https://docs.soliditylang.org/en/latest/contracts.html#function-modifiers] to add payable function modifier.
    function checkIn(uint256 spotId) external payable whenNotPaused { // User can enter the contract and must be payable to accept the deposit of user. 
        ...
        // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/cheatsheet.html#block-and-transaction-properties].
    ...
    function cancelCheckIn(uint256 spotId) external nonReentrant whenNotPaused { // allows driver to cancel check in ande get deposit back when sensor didnt signal occupied
        ...
        // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/control-structures.html#error-handling-assert-require-revert-and-exceptions] to transfer the amount to the specified address.
        // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/contracts.html#function-modifiers] to add keyword payable.
    ...
    function forceResetSpot(uint256 spotId) external onlyOwner nonReentrant { 
        ...
        // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/contracts.html#example]. Emits the event ForceReset and it is stored in the transaction logs.
    ...
    function forceEndParking(uint256 spotId) external onlyOwner nonReentrant { // Owner has the ability to manually settle the contract if oracle can not report free (sensor issues).
        ...
        // Subsequent code is taken from [https://docs.soliditylang.org/en/latest/contracts.html#example]. Emits the event ForceEnd and it is stored in the transaction logs.
